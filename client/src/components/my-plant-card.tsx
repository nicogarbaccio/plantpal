...
export default function MyPlantCard({ userPlant }: MyPlantCardProps) {
  const { status, statusColor } = useWaterStatus(userPlant);
  ...
  const waterPlantMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/user-plants/${userPlant.id}`] });
  ...
  const statusText = () => {
    if (status === "needs-water") {
      return userPlant.nextWaterDate && new Date(userPlant.nextWaterDate) < new Date()
  ...
  return (
    <Card className="bg-white rounded-[12px] overflow-hidden shadow-md">
      <div className="relative">
        <PlantStatusBanner status={status} statusText={statusText()} icon={getBannerIcon()} />
        <img 
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-start mb-2">
          <div>
            <h4 className="font-poppins font-medium text-lg">{userPlant.nickname}</h4>
            <p className="text-gray-500 text-sm font-lato italic">{userPlant.location}</p>
          </div>
          <DropdownMenu>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center">
            ...
            <span className="text-sm font-lato">{userPlant.plant?.lightRequirements || "Unknown"}</span>
          </div>
        </div>

        <div className="mb-4">
          <p className="text-sm font-lato text-gray-500 mb-1">
            Last Watered: <span className="text-charcoal">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Button 
            className={`${status === "needs-water" ? "bg-primary hover:bg-primary/90" : "bg-accent hover:bg-gray-200"} text-${status === "needs-water" ? "white" : "charcoal"} py-2 rounded-[12px] font-poppins text-sm`}
          ...
          <Link href={`/my-collection/${userPlant.id}`}>
            <Button variant="outline" className="w-full bg-accent hover:bg-gray-200 text-charcoal py-2 rounded-[12px] font-poppins text-sm">
              View Details
            </Button>
          </Link>
        </div>
